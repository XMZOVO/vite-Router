import { Scene, Engine, Camera, Mesh, CubeTexture, Vector3, SceneLoader, ActionManager, AmmoJSPlugin, DirectionalLight, ExecuteCodeAction, FreeCamera, GizmoManager, LightGizmo, Matrix, MeshBuilder, PhysicsImpostor, ShadowGenerator, Light, AxesViewer, UniversalCamera } from '@babylonjs/core'
...
export class BasicScene {
  ...
  async CreateEnviroment(): Promise<void> {
    ...
    //Ground
    dJRMeshes.meshes[30].checkCollisions = true
    dJRMeshes.meshes[30].position.set(0, 0, 0)
    ...
    dJRMeshes.meshes[30].setParent(ground)
  ...
  CreateObject(): void {
    const source = MeshBuilder.CreateSphere('source', { diameter: 0.05 }, this.scene)
    source.position.set(0.2, 1.2, -0.2)
