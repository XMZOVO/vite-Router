...
set_zoom2)?(this.zoom_ymin=-1111==f?this.ymin:f,this.zoom_ymax=-1111==g?this.ymax:g):(delete this.zoom_ymin,delete this.zoom_ymax));this.wheel_zoomy=1<this.getDimension()||!this.draw_content}};G.prototype.countStat=function(a){for(var d=this.isTProfile(),b=this.getHisto(),c=b.fXaxis,f=this.getSelectIndex("x","left"),e=this.getSelectIndex("x","right"),g=0,k=0,t=0,n=0,l=0,z,u,m=null,r=null,q=this.getFramePainter(),v={name:b.fName,meanx:0,meany:0,rmsx:0,rmsy:0,integral:0,entries:this.stat_entries,xmax:0,
wmax:0};f<e;++f)if(z=c.GetBinCoord(f+.5),!a||a(z)){d?(u=b.fBinEntries[f+1],n+=b.fArray[f+1],l+=b.fSumw2[f+1]):u=b.getBinContent(f+1);if(null===m||u>r)m=z,r=u;g+=u;k+=u*z;t+=u*z*z}!q.isAxisZoomed("x")&&0<b.fTsumw&&(g=b.fTsumw,k=b.fTsumwx,t=b.fTsumwx2);v.integral=g;0<g&&(v.meanx=k/g,v.meany=n/g,v.rmsx=Math.sqrt(Math.abs(t/g-v.meanx*v.meanx)),v.rmsy=Math.sqrt(Math.abs(l/g-v.meany*v.meany)));null!==m&&(v.xmax=m,v.wmax=r);return v};G.prototype.fillStatistic=function(a,d,b){if(this.isIgnoreStatsFill())return!1;
var c=this.getHisto(),f=this.countStat(),e=d%10,g=Math.floor(d/10)%10,k=Math.floor(d/100)%10,t=Math.floor(d/1E3)%10,n=Math.floor(d/1E4)%10,l=Math.floor(d/1E5)%10,z=Math.floor(d/1E6)%10,u=Math.floor(d/1E7)%10;d=Math.floor(d/1E8)%10;a.clearPave();0<e&&a.addText(f.name);this.isTProfile()?(0<g&&a.addText("Entries = "+a.format(f.entries,"entries")),0<k&&(a.addText("Mean = "+a.format(f.meanx)),a.addText("Mean y = "+a.format(f.meany))),0<t&&(a.addText("Std Dev = "+a.format(f.rmsx)),a.addText("Std Dev y = "+
a.format(f.rmsy)))):(0<g&&a.addText("Entries = "+a.format(f.entries,"entries")),0<k&&a.addText("Mean = "+a.format(f.meanx)),0<t&&a.addText("Std Dev = "+a.format(f.rmsx)),0<n&&a.addText("Underflow = "+a.format(0<c.fArray.length?c.fArray[0]:0,"entries")),0<l&&a.addText("Overflow = "+a.format(0<c.fArray.length?c.fArray[c.fArray.length-1]:0,"entries")),0<z&&a.addText("Integral = "+a.format(f.integral,"entries")),0<u&&a.addText("Skew = <not avail>"),0<d&&a.addText("Kurt = <not avail>"));b&&a.fillFunctionStat(this.findFunction("TF1"),
...
f.fY2NDC=.99-b.fTopMargin,f.fFillStyle=1001,T(a,f)}}};p.drawHistogram1D=function(a,d,b){var c=new G(a,d);return p.ensureTCanvas(c).then(function(){c.setAsMainPainter();c.decodeOptions(b);c.checkPadRange(!c.options.Mode3D);c.scanContent();c.createStat();return c.callDrawFunc()}).then(function(){return c.drawNextFunction(0)}).then(function(){!c.options.Mode3D&&c.options.AutoZoom&&c.autoZoom();c.fillToolbar();return c})};p.drawHistogram2D=ca;p.drawTF2=function(a,d,b){var c=new JSROOT.DrawOptions(b),
f=Q(d,c.check("NOSAVE"));b=c.empty()?"cont3":"SAME"===c.opt?"cont2 same":c.opt;return ca(a,f,b).then(function(a){a.tf2_typename=d._typename;a.tf2_nosave=c.check("NOSAVE");a.updateObject=function(a){if(!a||this.tf2_typename!=a._typename)return!1;Q(a,this.tf2_nosave,this.getHisto());return!0};return a})};p.drawHStack=function(a,d,b){if(!d.fHists||!d.fHists.arr)return null;var c=new I(a,d,b);return p.ensureTCanvas(c,!1).then(function(){c.decodeOptions(b);c.options.nostack||(c.options.nostack=!c.buildStack(d));
if(!c.options.same){d.fHistogram||(d.fHistogram=c.createHistogram(d));var f=c.getMinMax(c.options.errors||c.options.draw_errors),e=c.options.hopt+" axis";f&&(e+=";minimum:"+f.min+";maximum:"+f.max);return JSROOT.draw(a,d.fHistogram,e).then(function(a){c.firstpainter=a})}}).then(function(){return c.drawNextHisto(0)})};JSROOT.TPavePainter=A;JSROOT.THistPainter=y;JSROOT.TH1Painter=G;JSROOT.TH2Painter=E;JSROOT.THStackPainter=I;return JSROOT});
