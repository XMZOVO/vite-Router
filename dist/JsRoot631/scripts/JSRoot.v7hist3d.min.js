...
d;if(d)var e=d.geometry;else e=this.geometry.clone(),d=e.getAttribute("position").array,"z"===f?d[6]=d[3]=d[15]=g:d[4]=d[16]=d[13]=-g,d=new b.Mesh(e,new b.MeshBasicMaterial({color:65280,side:b.DoubleSide})),this.add(d);d=e.getAttribute("position").array;"z"==f?(d[2]=d[11]=d[8]=a[f],d[5]=d[17]=d[14]=c[f]):(d[0]=d[9]=d[12]=a[f],d[6]=d[3]=d[15]=c[f]);e.getAttribute("position").needsUpdate=!0;e.computeFaceNormals();return!0};return d};var I=new b.Object3D;I.position.set(0,L,u);I.rotation.x=.25*Math.PI;
I.xyid=2;a=l.createLineSegments(a,K);I.add(a);p.forEach(function(a){var c=a.boundingBox.max.x-a.boundingBox.min.x,d=a.center?a.grx-c/2:e-c;c=new b.Matrix4;c.set(z,0,0,d,0,z,0,(-x*z-1.5*g)*(a.gry||1),0,0,1,0,0,0,0,1);a=new b.Mesh(a,h);a.applyMatrix4(c);I.add(a)});f.zoom&&I.add(B("x",this.size_x3d));q.add(I);I=new b.Object3D;I.position.set(0,y,u);I.rotation.x=.75*Math.PI;I.add(new b.LineSegments(a.geometry,K));p.forEach(function(a){var c=a.boundingBox.max.x-a.boundingBox.min.x,d=a.center?a.grx+c/2:
e;c=new b.Matrix4;c.set(-z,0,0,d,0,z,0,(-x*z-1.5*g)*(a.gry||1),0,0,-1,0,0,0,0,1);a=new b.Mesh(a,h);a.applyMatrix4(c);I.add(a)});I.xyid=4;f.zoom&&I.add(B("x",this.size_x3d));q.add(I);p=[];z=1;x=0;a=[];for(r=this.yaxis;v.next();)t=v.grpos,A=1===v.kind,w=this.y_handle.format(v.tick,2),v.last_major()?r&&r.fTitle||(w="y"):null===w&&(A=!1,w=""),A&&(w=new b.TextGeometry(w,{font:JSROOT.threejs_font_helvetiker_regular,size:d,height:0,curveSegments:5}),w.computeBoundingBox(),D=w.boundingBox.max.x-w.boundingBox.min.x,
m=w.boundingBox.max.y-w.boundingBox.min.y,w.center=!0,x=Math.max(x,m),w.gry=t,p.push(w),v.last_major()||(m=v.next_major_grpos()-t,0<D&&(z=Math.min(z,.9*m/D)),this.y_handle.isCenteredLabels()&&(w.gry+=m/2))),a.push(0,t,0,A?-g:.6*-g,t,0);r&&r.fTitle&&(v=new b.TextGeometry(P.translateLaTeX(r.fTitle),{font:JSROOT.threejs_font_helvetiker_regular,size:d,height:0,curveSegments:5}),v.computeBoundingBox(),v.center=!1,v.grx=2,v.gry=(L+y)/2,p.push(v));if(!f.use_y_for_z){a=l.createLineSegments(a,K);var J=new b.Object3D;
J.position.set(c,0,u);J.rotation.y=-.25*Math.PI;J.add(a);p.forEach(function(a){var c=a.boundingBox.max.x-a.boundingBox.min.x,d=a.center?a.gry+c/2:y;c=new b.Matrix4;c.set(0,z,0,(-x*z-1.5*g)*(a.grx||1),-z,0,0,d,0,0,1,0,0,0,0,1);a=new b.Mesh(a,h);a.applyMatrix4(c);J.add(a)});J.xyid=3;f.zoom&&J.add(B("y",this.size_y3d));q.add(J);J=new b.Object3D;J.position.set(e,0,u);J.rotation.y=-.75*Math.PI;J.add(new b.LineSegments(a.geometry,K));p.forEach(function(a){var c=a.boundingBox.max.x-a.boundingBox.min.x,d=
a.center?a.gry-c/2:y-c;c=new b.Matrix4;c.set(0,z,0,(-x*z-1.5*g)*(a.grx||1),z,0,0,d,0,0,-1,0,0,0,0,1);a=new b.Mesh(a,h);a.applyMatrix4(c);J.add(a)});J.xyid=1;f.zoom&&J.add(B("y",this.size_y3d));q.add(J)}p=[];z=1;a=[];w=t=A=null;v=this.zaxis;r=0;this.size_z3d&&(A=[],t=[]);for(;k.next();){D=k.grpos;m=1==k.kind;var O=this.z_handle.format(k.tick,2);null===O&&(m=!1,O="");if(m&&O){O=new b.TextGeometry(O,{font:JSROOT.threejs_font_helvetiker_regular,size:d,height:0,curveSegments:5});O.computeBoundingBox();
var R=O.boundingBox.max.x-O.boundingBox.min.x,M=O.boundingBox.max.y-O.boundingBox.min.y;O.translate(-R,-M/2,0);O.grz=D;p.push(O);null!==w&&0<M&&(z=Math.min(z,.9*(D-w)/M));r=Math.max(r,R);w=D}A&&m&&A.push(c,0,D,e,0,D);t&&m&&t.push(0,L,D,0,y,D);a.push(0,0,D,m?g:.6*g,0,D)}A&&0<A.length&&(k=new b.LineDashedMaterial({color:0,dashSize:2,gapSize:2}),A=l.createLineSegments(A,k),A.position.set(0,y,0),A.grid=2,A.visible=!1,q.add(A),k=new b.LineSegments(A.geometry,k),k.position.set(0,L,0),k.grid=4,k.visible=
!1,q.add(k));t&&0<t.length&&(k=new b.LineDashedMaterial({color:0,dashSize:2,gapSize:2}),t=l.createLineSegments(t,k),t.position.set(e,0,0),t.grid=3,t.visible=!1,q.add(t),k=new b.LineSegments(t.geometry,k),k.position.set(c,0,0),k.grid=1,k.visible=!1,q.add(k));var G=[];a=l.createLineSegments(a,K);for(k={$jscomp$loop$prop$n$63:0};4>k.$jscomp$loop$prop$n$63;k={$jscomp$loop$prop$n$63:k.$jscomp$loop$prop$n$63},++k.$jscomp$loop$prop$n$63)G.push(new b.Object3D),p.forEach(function(a){return function(c){var d=
