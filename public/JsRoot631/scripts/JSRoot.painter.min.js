...
f));var n=e.prereq||"";"v7"==e.direct?n+=";v7gpad":e.direct&&(n+=";gpad");var p=e.script||"";p&&(p=p.split(";"));return n||p?JSROOT.require(n).then(function(){return JSROOT.loadScript(p)}).then(function(){var a=JSROOT.findFunction(l);if(!a)return Promise.reject(Error("Fail to find function "+l+" after loading "+n));e.func=a;return d()}):Promise.reject(Error("Prerequicities to load "+l+" are not specified"))};JSROOT.redraw=function(a,b,c){if(!b||"object"!==typeof b)return Promise.reject(Error("not an object in JSROOT.redraw"));
var d=m.getElementCanvPainter(a),e,h=null,k;b._typename&&(e=z("ROOT."+b._typename));e&&e.draw_field&&b[e.draw_field]&&(b=b[e.draw_field]);if(d)if(d.matchObjectType(b._typename))(k=d.redrawObject(b,c))&&(h=d);else for(e=0;e<d.painters.length;++e){var l=d.painters[e];if(l.matchObjectType(b._typename)&&(k=l.redrawObject(b,c))){h=l;break}}else(d=(new g(a)).getTopPainter())&&"function"==typeof d.redrawObject&&(k=d.redrawObject(b,c))&&(h=d);if(h)return k&&"object"==typeof k&&k.then||(k=Promise.resolve(!0)),
k.then(function(){return h});JSROOT.cleanup(a);return JSROOT.draw(a,b,c)};JSROOT.drawingJSON=function(a){return(a=m.getElementCanvPainter(a))?a.produceJSON():""};m.compressSVG=function(a){a=a.replace(/url\(&quot;#(\w+)&quot;\)/g,"url(#$1)").replace(/ class="\w*"/g,"").replace(/ pad="\w*"/g,"").replace(/ title=""/g,"").replace(/<g objname="\w*" objtype="\w*"/g,"<g").replace(/<g transform="translate\(\d+,\d+\)"><\/g>/g,"").replace(/<g><\/g>/g,"");a=a.replace(/<svg x="0" y="0" overflow="hidden" width="\d+" height="\d+" viewBox="0 0 \d+ \d+"><\/svg>/g,
